            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    .onChange(
                        of: contextDirectory,
                        {
                            ...
                            let dockerfileURL = url.appending(
                                path: "Dockerfile"
                            )
                            if FileManager.default.fileExists(
                                atPath: dockerfileURL.absolutePath
                            ) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    .onChange(
                        of: dockerFile,
                        {
                            guard let url = dockerFile, contextDirectory == nil
                            else {
                                return
                            }
                ...
                if showAdditionalSettings {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField(
                            text: $platformString,
                            prompt: Text("Ex: linux/amd64,linux/arm64"),
                            label: {}
                        )
                    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text(
                            "⭑ Anything with empty key will be removed when creating."
                        )
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
